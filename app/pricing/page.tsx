  ...
  return (
    <div>
      <section style={{ padding: '80px 40px 60px', textAlign: 'center' }}>
        <div style={{ fontSize: '11px', fontWeight: '600', color: '#4F6BFF', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '12px' }}>PRICING</div>
        ...
        <p style={{ fontSize: '16px', color: 'rgba(245,245,247,0.4)', marginBottom: '32px' }}>Start free. Upgrade when you are ready.</p>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px', background: 'rgba(245,245,247,0.04)', borderRadius: '10px', border: '1px solid rgba(245,245,247,0.08)' }}>
          {['Monthly', 'Yearly'].map((label, i) => (
            <button key={label} onClick={() => setYearly(i === 1)} style={{
              ...
              color: (i === 1) === yearly ? '#000000' : 'rgba(245,245,247,0.5)',
              fontSize: '13px', fontWeight: '500', fontFamily: 'inherit', transition: 'all 0.15s',
              display: 'flex', alignItems: 'center', gap: '6px'
            }}>
              {label}
              {i === 1 && <span style={{ fontSize: '10px', padding: '1px 5px', background: '#4F6BFF', color: '#fff', borderRadius: '4px', fontWeight: '700' }}>-20%</span>}
            </button>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '0 40px 60px', maxWidth: '1060px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', alignItems: 'start' }}>
          {PLANS.map(plan => (
            <div key={plan.id} style={{
              ...
              background: plan.highlight ? 'rgba(79,107,255,0.08)' : 'rgba(245,245,247,0.02)',
              border: `1px solid ${plan.highlight ? 'rgba(79,107,255,0.4)' : 'rgba(245,245,247,0.06)'}`,
              transform: plan.highlight ? 'scale(1.03)' : 'scale(1)'
            }}>
              {plan.highlight && (
                <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', padding: '3px 14px', background: '#4F6BFF', borderRadius: '20px', fontSize: '11px', fontWeight: '700', color: '#fff', whiteSpace: 'nowrap' }}>MOST POPULAR</div>
              )}
              ...
              <div style={{ fontSize: '12px', color: 'rgba(245,245,247,0.4)', marginBottom: '20px', lineHeight: '1.5' }}>{plan.desc}</div>
              <div style={{ marginBottom: '22px' }}>
                {plan.price.monthly === 0 ? (
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: '13px', color: 'rgba(245,245,247,0.4)' }}>/mo</span>
                    {yearly && <div style={{ fontSize: '12px', color: '#30D158', marginTop: '3px' }}>Save ₹{(plan.price.monthly - plan.price.yearly) * 12}/year</div>}
                  </div>
                )}
              </div>
              <Link href={plan.href} style={{
                display: 'block', textAlign: 'center', padding: '11px', borderRadius: '9px',
                background: plan.highlight ? '#4F6BFF' : 'rgba(245,245,247,0.06)',
                border: `1px solid ${plan.highlight ? 'transparent' : 'rgba(245,245,247,0.1)'}`,
                color: '#fff', textDecoration: 'none', fontSize: '14px', fontWeight: '600',
                marginBottom: '22px', boxShadow: plan.highlight ? '0 4px 20px rgba(79,107,255,0.35)' : 'none'
              }}>{plan.cta}</Link>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '9px' }}>
                {plan.features.map((f, i) => (
                  <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                    <Check size={13} style={{ color: plan.highlight ? '#93A5FF' : '#30D158', flexShrink: 0, marginTop: '1px' }} />
                    <span style={{ fontSize: '13px', color: 'rgba(245,245,247,0.55)', lineHeight: '1.4' }}>{f}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ marginTop: '20px', padding: '18px 22px', borderRadius: '12px', background: 'rgba(255,159,10,0.05)', border: '1px solid rgba(255,159,10,0.18)', display: 'flex', gap: '12px' }}>
          <span style={{ fontSize: '18px' }}>💡</span>
          <div>
            <div style={{ fontSize: '13px', fontWeight: '600', color: '#FFC369', marginBottom: '3px' }}>How free AI access works</div>
            <div style={{ fontSize: '12px', color: 'rgba(245,245,247,0.4)', lineHeight: '1.6' }}>Watch a 1-minute ad → get 1 hour of AI features free. Paid plans have zero ads.</div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '0 40px 100px', maxWidth: '680px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {FAQS.map((faq, i) => (
            <div key={i} style={{ padding: '18px 20px', borderRadius: '12px', background: 'rgba(245,245,247,0.02)', border: '1px solid rgba(245,245,247,0.06)' }}>
              <div style={{ fontSize: '14px', fontWeight: '600', color: '#fff', marginBottom: '6px' }}>{faq.q}</div>
              <div style={{ fontSize: '13px', color: 'rgba(245,245,247,0.45)', lineHeight: '1.6' }}>{faq.a}</div>
            </div>
          ))}
        </div>
      </section>
